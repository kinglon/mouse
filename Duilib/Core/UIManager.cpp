...
void CPaintManagerUI::Init(HWND hWnd)
{
    ...
    m_aPreMessages.Remove(m_aPreMessages.Find(this)); //modify by yjl，一个duilib窗口对象可能会重复进行窗口创建与删除，这样将导致增加多次删除1次出现野指针
...
void CPaintManagerUI::MessageLoop()
{
    ...
    while( ::GetMessage(&msg, NULL, 0, 0) ) {
		if (msg.hwnd == NULL)  //modify by yjl，出现过句柄为空的消息
		{
			OutputDebugString(L"recv one null window message");
			continue;
		}
...
CStdStringPtrMap* CPaintManagerUI::GetCustomFontSet(bool bShared)
{	
	if (bShared)
	{
		return &m_SharedResInfo.m_CustomFonts;
	}
	else
	{
		return &m_ResInfo.m_CustomFonts;
	}
}
