...
//add by yejinlong
IMessageHookUI* CWindowWnd::m_pMessageHook = NULL;
IUIMessageSpy* CWindowWnd::m_pUIMessageSpy = NULL;
...
HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, int x, int y, int cx, int cy, HMENU hMenu)
{
    ...
    if (m_hWnd == NULL)
    {
        HINSTANCE tempInstance = CPaintManagerUI::GetInstance();
    }
    return m_hWnd;
}
...
{ 
    ...
    if( pThis != NULL ) {
        ...
        if (m_pMessageHook && m_pMessageHook->MessageHook(pThis->GetHWND(), uMsg, wParam, lParam, result))
        {
            return result;
        }
        ...
        if (m_pUIMessageSpy)
        {
            m_pUIMessageSpy->OnHandleMessageBefore(pThis->GetHWND(), uMsg, wParam, lParam);           
        }
       
        result = pThis->HandleMessage(uMsg, wParam, lParam);
      
        if (m_pUIMessageSpy)
        {
            m_pUIMessageSpy->OnHandleMessageAfter(pThis->GetHWND(), uMsg, wParam, lParam);
        }
